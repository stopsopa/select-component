/**
 * Not used anymore:
 * just previous experimentation
 *
 * first version before select.ts was created...
 *
 * and now select.ts is also obsolite, due to better composite implementation composition/composite-select/CompositeManager.ts
 */
...
class SelectComponent extends HTMLElement {
  ...
  // holds just keys
  _selected = [];
  // holds full object always in sync with _selectedKeys
  _toggled = [];
  _isReady = false;
  constructor() {
    super();
  ...
  connectedCallback() {
    ...
    const webSelectComponent = shadow.querySelector(".web-select-component");
    const flexList = shadow.querySelector(".flex-list");
    const input = shadow.querySelector("input");
    const clearBtn = shadow.querySelector(".clear-btn");
    const options = shadow.querySelector(".options");
    const popoverEl = shadow.querySelector("[data-popover]");
    ...
    flexList.addEventListener("click", (e) => {
      ...
      if (removeBtn) {
        const element = removeBtn.closest(".element");
        if (element) {
    ...
    options.addEventListener("click", (e) => {
      const target = e.target;
      const targetEl = target.closest(".element");
      if (targetEl) {
    ...
    webSelectComponent.addEventListener("click", (e) => {
      ...
      const hideBtn = target.closest(`[popovertargetaction="hide"]`);
      if (hideBtn) {
        ...
        // Use 'any' type cast since hidePopover is somewhat newer for strict Typescript
        return;
      }
  ...
  setSelected(keys, dispatch = true) {
  ...
  _reconcileSelected() {
    ...
    for (const key of this._selectedKeys) {
      ...
      if (inOptions) {
        ...
        const newObj = { ...inOptions, new: false };
...
export { SelectComponent, select_default as default };
