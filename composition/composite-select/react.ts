...
type BaseCompositeSelectProps<T extends Item = Item> = {
  ...
  "options-onInputChange"?: (detail: { originalEvent: Event; value: string; previousValue?: string }) => void;
...
export type CompositeSelectProps<T extends Item = Item> = BaseCompositeSelectProps<T> & {
  [K in string as K extends `${string}-${string}`
    ? K extends keyof BaseCompositeSelectProps<T>
      ? never
      : K
    : never]?: never;
};

export const CompositeSelect = React.forwardRef(
  <T extends Item = Item>(props: BaseCompositeSelectProps<T>, ref: React.ForwardedRef<HTMLElement>) => {
    const {
...
) as <T extends Item = Item>(props: CompositeSelectProps<T> & React.RefAttributes<HTMLElement>) => React.ReactElement;
