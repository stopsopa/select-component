...
const init = (initialSelected: DemoItem[] = [], states: Partial<DemoState> = {}) => {
  ...
  section.innerHTML = `
    ...
    <div class="controls gcp-css" style="margin-bottom: 8px;">
      <div class="controls-label">SelectedSectionManager (Web Component)</div>
  ...
  optStringRenderBtn.addEventListener("click", () => {
    mgr.setRenderItem((item: any) => {
      ...
      el.innerHTML = `<strong>WC: ${item.label}</strong> <span data-remove="${item.id}" style="cursor:pointer; margin-left: 5px;">[x]</span>`;
      return el;
    });
  });
