...
const css2 = await fetch("./SelectedSectionManager.css").then((r) => r.text());
// OptionsSection.cssText = css1 + "\n" + css2;
(SelectedSection as any).cssText = css2;
...
const init = (initialSelected: DemoItem[] = [], states: Partial<DemoState> = {}) => {
  ...
  const resizerCenter = states.center || "450px";
  ...
  section.innerHTML = `
    ...
    <div class="controls gcp-css" style="margin-bottom: 8px;">
      <div class="controls-label">SelectedSectionManager (No CSS Request)</div>
  ...
  const labelInputSel = section.querySelector('[data-role="label-input-opt"]') as HTMLInputElement;
  const valueInputSel = section.querySelector('[data-role="value-input-opt"]') as HTMLInputElement;
  ...
  const syncUrl = () => {
    ...
    urlStateConfig.toUrl(url, id, {
      ...
      center: resizer.getAttribute("center") || "450px",
  ...
  addBtn.addEventListener("click", () => {
    const id = getNextId();
    setNextId(id + 1);
    mgr.setSelected([...mgr.getSelected(), { id, label: `NC Item ${id}` }]);
    syncUrl();
  });
  ...
  optRenderBtn.addEventListener("click", () => {
    mgr.setRenderItem((item: any, def: any) => {
      const el = def(item);
      if (item.color) {
  ...
  optStringRenderBtn.addEventListener("click", () => {
    mgr.setRenderItem((item: any) => {
      ...
      el.innerHTML = `<strong>NC: ${item.label}</strong> <span data-remove="${item.id}" style="cursor:pointer; margin-left: 5px;">[x]</span>`;
      return el;
    });
  });
