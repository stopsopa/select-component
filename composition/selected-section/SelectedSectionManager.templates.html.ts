...
const init = (initialSelected: DemoItem[] = [], states: Partial<DemoState> = {}) => {
  ...
  const optListRenderBtn = section.querySelector('[data-role="opt-list-render-btn"]') as HTMLButtonElement;
  ...
  optRenderBtn.addEventListener("click", () => {
    mgr.setRenderItem((item: DemoItem) => {
  ...
  optListRenderBtn.addEventListener("click", () => {
    mgr.setRenderList((selected: DemoItem[], defaultRenderList: any) => {
